...
data Token = MOV | LOAD | STORE | ADD | SUB | DIV | MUL | NEGATE | EQUAL | NOT | GTHAN |
             JMP | BRZ | BRNZ | BRGT | BRGE | READ | WRITE | CALL | RET | EXIT |
             REG String | LIT Int | FPtok | SPtok | BPtok | SEMICOL | COLON | ENDOFINPUT
                                   deriving (Eq,Show)

front3addr :: String -> Maybe [ThreeAddr]
front3addr = lexer Common.<> parse3addr
...
lexAlpha cs = let (as,rest) = span isAlpha cs in
 case as of
      ...
      "brgt"   -> BRGT <:> lexer rest
      "brge"   -> BRGE <:> lexer rest
...
parse3addr (t:ts) = case t of
  ...
  BRGT   -> do
              (r,a,ts') <- twoaddress ts
              rest      <- parse3addr ts'
              return $ BrGT r a : rest
  BRGE   -> do
              (r,a,ts') <- twoaddress ts
              rest      <- parse3addr ts'
              return $ BrGE r a : rest
  LIT i  -> do
              ts'  <- colon ts
              rest <- parse3addr ts'
              return $ Label i : rest              
